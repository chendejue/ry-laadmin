...
use RuiYi\LaAdmin\Http\Actions\Extensions\InstallFromLocal;
use RuiYi\LaAdmin\Http\Actions\Extensions\Marketplace;
use RuiYi\LaAdmin\Http\Displayers\Extensions;
use RuiYi\LaAdmin\Http\Repositories\Extension;
use RuiYi\LaAdmin\Layout\Content;
use RuiYi\LaAdmin\Support\Helper;
use RuiYi\LaAdmin\Support\StringOutput;
use Illuminate\Routing\Controller;
use Illuminate\Support\Facades\Artisan;
use Illuminate\Contracts\Support\Renderable;
use RuiYi\LaAdmin\Form\NestedForm;
use RuiYi\LaAdmin\Widgets\Alert;
use RuiYi\LaAdmin\Widgets\Tab;
use RuiYi\LaAdmin\Http\Controllers\AdminController;
...
class ExtensionController extends AdminController
{
        ...
        return new Grid(new Extension(), function (Grid $grid) {
            ...
            $grid->quickCreate(function (Grid\Tools\QuickCreate $create) {
                $create->text('name')
                    ...
                    ->placeholder('Input Name. Eg: RuiYi-LaAdmin/demo')
                    ->required();
                $create->text('namespace')
                    ->attribute('style', 'width:240px')
                    ->placeholder('Input Namespace. Eg: RuiYi-LaAdmin\\Demo');
                $create->select('type')
    ...
    protected function form()
    {
        return Form::make(new Extension(), function (Form $form) {
            ...
            $form->text('name','包名')->help('例如: RuiYi-LaAdmin/demo')->rules(function () {
                return [
                    'required',
