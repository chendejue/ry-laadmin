...
namespace RuiYi\LaAdmin;

use Closure;
use RuiYi\LaAdmin\Contracts\ExceptionHandler;
use RuiYi\LaAdmin\Contracts\Repository;
use RuiYi\LaAdmin\Exception\InvalidArgumentException;
use RuiYi\LaAdmin\Http\Controllers\AuthController;
use RuiYi\LaAdmin\Http\JsonResponse;
use RuiYi\LaAdmin\Layout\Menu;
use RuiYi\LaAdmin\Layout\Navbar;
use RuiYi\LaAdmin\Layout\SectionManager;
use RuiYi\LaAdmin\Repositories\EloquentRepository;
use RuiYi\LaAdmin\Support\Composer;
use RuiYi\LaAdmin\Support\Helper;
use RuiYi\LaAdmin\Traits\HasAssets;
use RuiYi\LaAdmin\Traits\HasHtml;
use RuiYi\LaAdmin\Traits\HasPermissions;
...
class Admin
{
    ...
    {
        return sprintf('RuiYi LaAdmin <comment>version</comment> <info>%s</info>', \RuiYi\LaAdmin\Support\Helper::getPackageVersion('RuiYi/laAdmin'));
    }
     ...
     * 配置.
     *
     * @return \RuiYi\LaAdmin\Support\Setting
     */
     ...
     * 上下文管理.
     *
     * @return \RuiYi\LaAdmin\Support\Context
     */
     ...
     * 翻译器.
     *
     * @return \RuiYi\LaAdmin\Support\Translator
     */
     ...
     *
     * @param  string  $name
     * @return \RuiYi\LaAdmin\Extend\Manager|\RuiYi\LaAdmin\Extend\ServiceProvider|null
     */
    ...
    public static function routes()
    {
        ...
        if (config('admin.auth.enable', true)) {
            app('router')->group($attributes, function ($router) {
                ...
                $router->namespace('RuiYi\LaAdmin\Http\Controllers')->group(function ($router) {
                    /* @var \Illuminate\Routing\Router $router */
                ...
                $router->resource('auth/extensions', 'RuiYi\LaAdmin\Http\Controllers\ExtensionController');
                ...
                // form-media
                $router->any('lake-form-media/get-files', \RuiYi\LaAdmin\Form\Extend\FormMedia\Controllers\FormMedia::class.'@getFiles')->name('admin.lake-form-media.get-files');
                $router->post('lake-form-media/upload', \RuiYi\LaAdmin\Form\Extend\FormMedia\Controllers\FormMedia::class.'@upload')->name('admin.lake-form-media.upload');
                $router->post('lake-form-media/create-folder', \RuiYi\LaAdmin\Form\Extend\FormMedia\Controllers\FormMedia::class.'@createFolder')->name('admin.lake-form-media.create-folder');
                // sku-image
                $router->resource('sku-action', \RuiYi\LaAdmin\Http\Controllers\SkuAttributeController::class);
                $router->post('sku-image-upload', \RuiYi\LaAdmin\Form\Extend\Sku\Controllers\UploadController::class.'@store')->name('admin.sku-image-upload');
                $router->post('sku-image-remove', \RuiYi\LaAdmin\Form\Extend\Sku\Controllers\UploadController::class.'@delete')->name('admin.sku-image-remove');
    ...
    {
        $attributes = [
            'prefix'     => admin_base_path('ruiyi-api'),
            'middleware' => config('admin.route.middleware'),
    ...
    public static function registerHelperRoutes()
    {
        ...
        app('router')->group($attributes, function ($router) {
            /* @var \Illuminate\Routing\Router $router */
            $router->get('helpers/scaffold', 'RuiYi\LaAdmin\Http\Controllers\ScaffoldController@index');
            $router->post('helpers/scaffold', 'RuiYi\LaAdmin\Http\Controllers\ScaffoldController@store');
            $router->post('helpers/scaffold/table', 'RuiYi\LaAdmin\Http\Controllers\ScaffoldController@table');
            $router->get('helpers/icons', 'RuiYi\LaAdmin\Http\Controllers\IconController@index');
        });
    }
}
