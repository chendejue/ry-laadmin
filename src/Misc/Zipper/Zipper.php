...
namespace RuiYi\LaAdmin\Misc\Zipper;

use RuiYi\LaAdmin\Misc\Zipper\Repositories\RepositoryInterface;
use Exception;
...
class Zipper
{
    ...
    public function make($pathToFile, $type = 'zip')
    {
        ...
        $objectOrName = $type;
        if (is_string($type)) {
            $objectOrName = 'RuiYi\LaAdmin\Misc\Zipper\Repositories\\' . ucwords($type) . 'Repository';
        }

        if (!is_subclass_of($objectOrName, 'RuiYi\LaAdmin\Misc\Zipper\Repositories\RepositoryInterface')) {
